...
- Constexpr <cmath> (e.g. `std::abs()` ), and <cstdlib> ([[6-7|6.7 -- Relational operators and floating point comparisons]]).
...
- Literal suffixes for `std::size_t` and the corresponding signed type ([[5-2|5.2 -- Literals]]).
- Multidimensional subscript `operator[]` (Mentioned in lesson [[17-13|17.13 -- Multidimensional std::array]]).
- Multidimensional span `std::mdspan` ([[17-13|17.13 -- Multidimensional std::array]]).
...
- `std::to_underlying` to get the underlying type of enum ([[13-6|13.6 -- Scoped enumerations (enum classes)]]).
- `std::unreachable()` (no lesson yet).
- Using unknown pointers and references in constant expressions ([[17-2|17.2 -- std::array length and indexing]]).
