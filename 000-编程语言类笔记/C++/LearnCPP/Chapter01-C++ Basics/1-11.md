...
This is the preferred solution of the bunch. When std::cout executes, the expression _num _ 2* will get evaluated, and the result will be double *num*‘s value. That value will get printed. The value in *num\* itself will not be altered, so we can use it again later if we wish.
