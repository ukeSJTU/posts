...
In lesson [[1-10|1.10 -- Introduction to expressions]], we introduced expressions. By default, expressions evaluate at runtime. And in some cases, they must do so:
...
In prior lesson [[5-4|5.4 -- The as-if rule and compile-time optimization]], we discussed the as-if rule, and how the compiler can optimize programs by shifting work from runtime to compile-time. Under the as-if rule, the compiler may choose whether to evaluate certain expressions at runtime or compile-time:
...
We’ll discuss constexpr variables in the next lesson ([[5-6|5.6 -- Constexpr variables]]), when we cover constexpr variables.

> **For advanced readers**
>
> A few common cases where a compile-time evaluatable expression is required:
>
> - The initializer of a constexpr variable ([[5-6|5.6 -- Constexpr variables]]).
> - A non-type template argument ([[11-9|11.9 -- Non-type template parameters]]).
> - The defined length of a `std::array` ([[17-1|17.1 -- Introduction to std::array]]) or a C-style array ([[17-7|17.7 -- Introduction to C-style arrays]]).
...
- Constexpr variables (discussed in upcoming lesson [[5-6|5.6 -- Constexpr variables]]).
...
- Templates (introduced in lesson [[11-6|11.6 -- Function templates]]).
- static_assert (discussed in lesson [[9-6|9.6 -- Assert and static_assert]]).
...
>
> - Non-type template parameters (see [[11-9|11.9 -- Non-type template parameters]]).
> - Enumerators (see [[13-2|13.2 -- Unscoped enumerations]]).
> - Type traits (see the [cppreference page for type traits](https://en.cppreference.com/w/cpp/header/type_traits)).
> - Constexpr lambda expressions (see [[20-6|20.6 -- Introduction to lambdas (anonymous functions)]]).
...
>
> - Non-const variables.
> - Const non-integral variables, even when they have a constant expression initializer (e.g. `const double d { 1.2 };` ). To use such variables in a constant expression, define them as constexpr variables instead (see lesson [[5-6|5.6 -- Constexpr variables]]).
...
Per the definition of a compile-time constant, only const _integral_ variables with a constant expression initializer are compile-time constants. `c` is a double, which is not an integral type, so it does not meet this definition.
