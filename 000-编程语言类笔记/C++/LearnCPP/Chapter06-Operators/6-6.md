...
> **Related content**
>
> We cover if-else statements in lesson [[4-10|4.10 -- Introduction to if statements]].
...
> **Related content**
>
> We cover the way that C++ prioritizes the evaluation of operators in future lesson [[6-1|6.1 -- Operator precedence and associativity]].
...
>
> Alternatively, one or both of the second and third operands is allowed to be a throw expression. We cover `throw` in lesson [[27-2|27.2 -- Basic exception handling]].
...
> **Related content**
>
> The surprising case above related to mixing signed and unsigned values is due to the arithmetic conversion rules, which we cover in lesson [[10-5|10.5 -- Arithmetic conversions]].
...
> Since we haven’t covered `if constexpr` yet (we do so in lesson [[8-4|8.4 -- Constexpr if statements]]), `x` is non-constexpr in this example to avoid the potential compiler warning.
