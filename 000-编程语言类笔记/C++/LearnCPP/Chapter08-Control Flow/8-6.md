...
This lesson continues our exploration of switch statements that we started in the prior lesson [[8-5|8.5 -- Switch statement basics]]. In the prior lesson, we mentioned that each set of statements underneath a label should end in a `break statement` or a `return statement` .
