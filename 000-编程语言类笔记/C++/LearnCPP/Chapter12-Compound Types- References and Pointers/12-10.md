...
In prior lessons, we’ve covered two different ways to pass an argument to a function: pass by value ([[2-4|2.4 -- Introduction to function parameters and arguments]]) and pass by reference ([[12-5|12.5 -- Pass by lvalue reference]]).
...
For many of the same reasons we typically don’t `const` regular (non-pointer, non-reference) function parameters (discussed in [[5-1|5.1 -- Constant variables (named constants)]]), we also typically don’t `const` pointer function parameters. Let’s make two assertions:
...
In most cases, it is more effective to do the opposite: test whether the function parameter is null as a precondition ([[9-6|9.6 -- Assert and static_assert]]) and handle the negative case immediately:
...
If a null pointer should never be passed to the function, an `assert` (which we covered in lesson [[9-6|9.6 -- Assert and static_assert]]) can be used instead (or also) (as asserts are intended to document things that should never happen):
