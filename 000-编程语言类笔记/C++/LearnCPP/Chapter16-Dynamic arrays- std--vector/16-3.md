...
In the prior lesson [[16-2|16.2 -- Introduction to std::vector and list constructors]], we introduced `operator[]` , which can be used to subscript an array to access an element.
...
Before we proceed further, let’s quickly recap some material from lesson [[10-4|10.4 -- Narrowing conversions, list initialization, and constexpr initializers]] regarding sign conversions (integral conversions from signed to unsigned or vice-versa) because we’ll be talking about these a lot in this chapter.
...
However, if the value to be sign converted is constexpr and can be converted to an equivalent value in the opposing type, the sign conversion is _not_ considered to be narrowing. This is because the compiler can guarantee that the conversion is safe, or halt the compilation process.
...
> **Related content**
>
> We cover nested typedefs in lesson [[15-3|15.3 -- Nested types (member types)]].
...
> We discuss C-style array decay in lesson [[17-8|17.8 -- C-style array decay]].
...
However, as per our best practices ([[4-5|4.5 -- Unsigned integers, and why to avoid them]]), we generally want to avoid using unsigned types to hold quantities.
