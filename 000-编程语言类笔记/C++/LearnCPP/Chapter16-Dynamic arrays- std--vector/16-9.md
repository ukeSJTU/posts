...
> We discuss this further in lesson [[17-3|17.3 -- Passing and returning std::array]].
...
A better option is to use the helper function that we introduced in lesson [[13-6|13.6 -- Scoped enumerations (enum classes)]], which allows us to convert the enumerators of enum classes to integral values using unary `operator+` .
