...
In C++20, the `std::ssize()` non-member function, which returns the length as a large _signed_ integral type (usually `std::ptrdiff_t` , which is the type normally used as the signed counterpart to `std::size_t` ).
...
Write definitions for the following. Use CTAD where possible ([[13-14|13.14 -- Class template argument deduction (CTAD) and deduction guides]]).
...
- We’ll start by defining our list of words and writing a random word picker. You can use the Random.h from lesson [[8-15|8.15 -- Global random numbers (Random.h)]] to assist.
...
Hint: It’s easier and faster to use a
`bool`
for each letter than it is to keep a list of letters and have to search that list to determine whether a letter exists.

<details>
<summary>Show Solution</summary>

Hint: It is okay to use
`std::vector<bool>`
here since we won’t be using any of the other functionality of the standard library.

<details>
<summary>Show Solution</summary>

Hint: You can convert a letter into an array index via
`(letter % 32)-1`
