...
In lesson [[16-3|16.3 -- std::vector and the unsigned length and subscript problem]], we discussed the unfortunate decision to make the standard library container classes use unsigned values for lengths and indices. Because `std::array` is a standard library container class, it is subject to the same issues.
...
Before proceeding, now would be a good time to refresh your memory on “sign conversions are narrowing conversions, except when constexpr” (see [[16-3|16.3 -- std::vector and the unsigned length and subscript problem]]).
...
> **Related content**
>
> We cover class templates (which includes struct templates) in lesson [[13-13|13.13 -- Class templates]] and non-type template parameters in lesson [[11-9|11.9 -- Non-type template parameters]].
...
> A workaround is to make  `foo()`  a function template where the array length is a non-type template parameter. This non-type template parameter can then be used inside the function. We discuss this further in lesson [[17-3|17.3 -- Passing and returning std::array]].
> ```cpp
...
In the prior lesson [[17-1|17.1 -- Introduction to std::array]], we covered that the most common way to index a `std::array` is to use the subscript operator ( `operator[]` ). No bounds checking is done in this case, and passing in an invalid index will result in undefined behavior.
...
However, if either of these functions are called with a non-constexpr signed integral value, the conversion to `std::size_t` is considered narrowing and your compiler may emit a warning. We discuss this case further (using `std::vector` ) in lesson [[16-3|16.3 -- std::vector and the unsigned length and subscript problem]].
