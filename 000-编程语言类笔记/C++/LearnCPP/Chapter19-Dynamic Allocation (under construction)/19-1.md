...
Although the syntax makes it look like we’re deleting a _variable_, this is not the case! The pointer variable still has the same scope as before, and can be assigned a new value (e.g. `nullptr` ) just like any other variable.
