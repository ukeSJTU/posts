...
在第 [[12-7|12.7 -- 指针入门]] 课中，你学习了指针是一种保存另一个变量地址的变量。函数指针与此类似，不同之处在于它们指向的是函数而不是变量！
...
标识符 `foo()` 是该函数的名称。但这个函数的类型是什么？函数有自己的函数类型——在本例中，是返回整数且不带参数的函数类型。与变量类似，函数在内存中也有分配的地址（因此它们是左值）。

When a function is called (via `operator()` ), execution jumps to the address of the function being called:
当调用一个函数（通过 `operator()`）时，程序会跳转到被调用函数的地址执行：
...
在你的编程生涯中（如果还没有遇到的话），你很可能会犯一个简单的错误：
...
我们本意是调用函数 `foo()` 并打印其返回值，但实际上却直接将函数 `foo` 传递给了 `std::cout`。这种情况下会发生什么？
...
当以名称（不带括号）引用函数时，C++ 会将该函数转换为函数指针（即保存函数地址）。然后 `operator<<` 试图打印这个函数指针，但它无法做到，因为 `operator<<` 不知道如何打印函数指针。标准规定，在这种情况下，`foo` 应该被转换为 `bool` 类型（`operator<<` 知道如何打印布尔值）。由于 `foo` 的函数指针是非空指针，因此它总是被评估为布尔值 `true`。因此，输出结果应为：
...
> 某些编译器（如 Visual Studio）有编译器扩展，会直接打印函数的地址：
>
> ````plaintext
> 0x002717f0
...
> 如果你的平台没有打印函数地址，而你希望这样做，可以通过将函数转换为 void 指针并打印来实现：
> ```cpp
> #include <iostream>
>
> int foo() // code starts at memory address 0x002717f0
...
就像可以声明指向普通变量的非常量指针一样，也可以声明指向函数的非常量指针。在本课的其余部分，我们将探讨这些函数指针及其用法。函数指针是一个相当高级的话题，如果你只关心 C++ 基础内容，可以跳过或略读本课剩余内容。
