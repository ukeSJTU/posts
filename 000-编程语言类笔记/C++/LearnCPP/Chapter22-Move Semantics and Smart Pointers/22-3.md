...
In lesson [[22-1|22.1 -- Introduction to smart pointers and move semantics]], we took a look at std::auto_ptr, discussed the desire for move semantics, and took a look at some of the downsides that occur when functions designed for copy semantics (copy constructors and copy assignment operators) are redefined to implement move semantics.
...
> We introduce `noexcept` in lesson [[27-9|27.9 -- Exception specifications and noexcept]] and discuss why move constructors and move assignment are marked as `noexcept` in lesson [[27-10|27.10 -- std::move_if_noexcept]].
...
In lesson [[21-12|21.12 -- Overloading the assignment operator]], we mentioned the copy and swap idiom. Copy and swap also works for move semantics, meaning we can implement our move constructor and move assignment by swapping resources with the object that will be destroyed.
