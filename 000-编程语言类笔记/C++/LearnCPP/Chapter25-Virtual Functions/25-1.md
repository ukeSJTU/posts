...
> 1. This function does not enforce that  `T`  is an  `Animal` . Rather, it will accept an object of any type that contains a  `getName()`  and  `speak()`  member function, whether that makes sense or not.
>
> ````
