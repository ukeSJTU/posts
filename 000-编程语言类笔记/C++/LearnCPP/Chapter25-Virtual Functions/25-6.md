...
In this case, when b is created, b.**vptr points to Base’s virtual table, not D1’s virtual table. Since bPtr is pointing to b, `bPtr->**vptr`  points to Base’s virtual table as well. Base’s virtual table entry for function1() points to Base::function1(). Thus,  `bPtr->function1()` resolves to Base::function1(), which is the most-derived version of function1() that a Base object should be able to call.
