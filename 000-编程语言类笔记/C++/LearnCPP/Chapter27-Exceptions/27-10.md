...
In lesson [[22-4|22.4 -- std::move]], we covered `std::move` , which casts its lvalue argument to an rvalue so that we can invoke move semantics. And in lesson [[27-9|27.9 -- Exception specifications and noexcept]], we covered the `noexcept` exception specifier and operator. This lesson builds on both concepts.
