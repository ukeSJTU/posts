...
In C++, all functions are classified as either _non-throwing_ or _potentially throwing_. A **non-throwing function** is one that promises not to throw exceptions that are visible to the caller. A **potentially throwing function** may throw exceptions that are visible to the caller.
...
> **Optional reading**
>
> Before C++11, and until C++17, _dynamic exception specifications_ were used in place of `noexcept` . The **dynamic exception specifications** syntax uses the `throw` keyword to list which exception types a function might directly or indirectly throw:
>
> ````cpp
